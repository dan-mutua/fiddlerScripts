...
class Handlers
{
    ...
    */
    ...
    // Add custom columns for X-HOSTIP and HTTPS-CLIENT-SNIHOSTNAME
   

    // Add a custom column to display FLAGS
    public static BindUIColumn("FLAGS", 100)
    function FillFlagsColumn(oS: Session): String {
        var flags = "";
        for (var sFlag in oS.oFlags) {
            flags += sFlag.Key + ": " + sFlag.Value + "; ";
        }
        return flags.Trim();
    }
    ...
    static function FlagsReport(arrSess: Session[]):String {
        ...
        for (var i:int = 0; i<arrSess.Length; i++) {
            oSB.AppendLine("SESSION DATA");
            oSB.AppendFormat("ID: {0}\n", arrSess[i].id);
            oSB.AppendFormat("URL: {0}\n", arrSess[i].fullUrl);
            oSB.AppendFormat("Method: {0}\n", arrSess[i].RequestMethod);
            oSB.AppendFormat("Host: {0}\n", arrSess[i].host);
            oSB.AppendFormat("Client IP: {0}\n", arrSess[i].clientIP);
            oSB.AppendLine("FLAGS:");
        ...
        var filePath = "C:\Users\v-dmutua\Documents\Fiddler2\Captures\sessions_data.txt";
        File.WriteAllText(filePath, oSB.ToString());
    ...
    */
    ...
    static function OnBeforeRequest(oSession: Session) {
        ...
        var clientIP = oSession.oRequest["X-Forwarded-For"];
        if (!clientIP || clientIP == "127.0.0.1") {
            // Fallback to the default client IP
            clientIP = oSession.clientIP;
        }

        // If the clientIP is still 127.0.0.1, attempt to resolve the actual IP
        if (clientIP == "127.0.0.1") {
            try {
                ...
                // Iterate through the address list to find the first IPv4 address
                ...
                FiddlerObject.StatusText = "Error resolving local IP: " + e.message;
            }
        }

        // Set the X-HOSTIP header with the resolved client IP
        //oSession.oRequest["X-HOSTIP"] = clientIP;
       // oSession.oRequest["HTTPS-CLIENT-SNIHOSTNAME"] = oSession["https-client-sn"];
        ...
        if (m_SimulateModem) {
            oSession["request-trickle-delay"] = "300"; // Delay sends by 300ms per KB uploaded
            oSession["response-trickle-delay"] = "150"; // Delay receives by 150ms per KB downloaded
        }
...
*/
    ...
    */
    ...
    static function Main() {
       ...
       // UI.lvSessions.AddBoundColumn("X-HOSTIP", 60, "@request.X-HOSTIP");
       // UI.lvSessions.AddBoundColumn("HTTPS-CLIENT-SNIHOSTNAME", 60, "@request.HTTPS-CLIENT-SNIHOSTNAME");
